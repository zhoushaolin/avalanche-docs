...
如果一个节点收到一个投给某个顶点的票，它当作是对一个顶点中所有事务的投票，并且投票向上传递应用。 当顶点中的所有事务都被接受时，这个顶点就被接受了。 如果一个顶点包含一个被拒绝的事务，那么它也将被拒绝，它的所有后代也将被拒绝。 如果一个顶点被拒绝，所有有效的事务将重新发布到一个的新顶点中, 这个定点是没被拒绝的顶点的子顶点。 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY5MjUyMDA1NiwtODI3NTI2ODgwLDE0MD
