...
另一种省钱的方法是使用**Reserved Instance**。使用这个实例，您得提前支付一整年的EC2使用费，并按较低的每小时费率换取锁定。如果您想要长时间运行一个节点，并且不想冒服务中断的风险，那么这是一个很好的省钱选择。同样，选择该选项前，请事先自行调研。

另一种省钱的方法是使用预留实例。使用预留实例时，您需要提前支付一整年的EC2使用费用，并通过锁定获得较低的每小时费率。如果您打算长时间运行一个节点，并且不想冒服务中断的风险，那么这是一个节省资金的好选择。同样，在选择这个选项之前要自己做调查。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0OTE4MDc3MDEsLTYwMDkwMTczMiwtNj
k5OTAwOTAzXX0=
-->