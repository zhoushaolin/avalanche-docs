---
description: 'Tutorial generated by community Member: Seq'
---
...
This opens the “nano” text editor. We now copy and paste the following into the text editor. We edit the file in the place where it says, “IPv4PublicIPAddress” and put in our “IPv4 Public IP” copied from the Azure Portal, then hit Ctrl-x to save the file and exit.
...
If you didn’t configure a static IP address, the IP address will change each time the VM is stopped and started and this file would require to be updated each time with the new IP. To avoid this issue, make sure you use a Static IP as described earlier.
