...
* `initialHolders`(初始持有人)中的每个元素都指定了这个地址持有的地址在初始时持有的金额单位。Each element in `initialHolders` specifies that `address` holds `amount` units of the asset at genesis.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDc0MjA2NjgwLC0xODY2NTY5MjA2LDE5Nz
