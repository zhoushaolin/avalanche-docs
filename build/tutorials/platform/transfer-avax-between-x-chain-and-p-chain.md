...
To send the AVAX, call the X-Chain’s [`avm.exportAVAX`](../../apis/exchange-chain-x-chain-api.md#avm-exportavax) method.
...
where `to` is the address of a P-Chain address you hold and `changeAddr` is the address to send any change to. You can leave `changeAddr` blank; if you leave it blank, change will be returned to an address controlled by your user \(see [here](../../apis/platform-chain-p-chain-api.md#platform-createaddress) for instructions on creating a new P-Chain address\).
...
We can verify that this transaction was accepted by calling [`avm.getTxStatus`](../../apis/exchange-chain-x-chain-api.md#avm-gettxstatus):
...
We can also call [`avm.getBalance`](../../apis/exchange-chain-x-chain-api.md#avm-getbalance) to check that the AVAX was deducted from an address held by our user:
...
Our transfer isn’t done just yet. We need to call the P-Chain’s [`platform.importAVAX`](../../apis/platform-chain-p-chain-api.md#platform-importavax) method to finish the transfer.
...
To do so, call [`platform.exportAVAX`](../../apis/platform-chain-p-chain-api.md#platform-exportavax):
...
This returns the transaction ID, and we can check that the transaction was committed with another call to [`platform.getTxStatus`](../../apis/platform-chain-p-chain-api.md#platform-gettxstatus). Again, make sure that the amount you’re sending exceeds the transaction fee.

## Import AVAX to the X-Chain from the P-Chain

To finish our transfer from the P-Chain to the X-Chain, call [`avm.importAVAX`](../../apis/exchange-chain-x-chain-api.md#avm-importavax):
...
Note that `to` is the same address specified in our call to [`platform.exportAVAX`](../../apis/platform-chain-p-chain-api.md#platform-exportavax).

Just as before, we can call [`avm.getBalance`](../../apis/exchange-chain-x-chain-api.md#avm-getbalance) to verify the funds were received. The balance should have increased by `3,000,000` minus the transaction fee.
...
Now, you can use the address corresponding to the private key you exported and switch to using the C- prefix in the [`avm.exportAVAX`](../../apis/exchange-chain-x-chain-api.md#avm-exportavax) call:
...
Lastly, we can finish up by importing AVAX from the C-Chain to the X-Chain, call [`avm.importAVAX`](../../apis/exchange-chain-x-chain-api.md#avm-importavax).
