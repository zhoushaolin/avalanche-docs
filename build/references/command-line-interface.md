...
If set to `true`, API calls require an authorization token. Defaults to `false`. See [here](../apis/auth-api.md) for more information.

`--api-auth-password` \(string\):

The password needed to create/revoke authorization tokens. If `--api-auth-required=true`, must be specified; otherwise ignored. See [here](../apis/auth-api.md) for more information.
