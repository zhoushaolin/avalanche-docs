...
最多只能从每个 `conn-meter-reset-duration`给定的IP升级`conn-meter-max-conns`连接  每次“conn-meter-reset-duration”最多从给定IP升级“conn-meter-max-conn”连接。如果' conn-meter-reset-duration '为0，则传入连接不受速率限制。
Upgrade at most `conn-meter-max-conns` connections from a given IP per `conn-meter-reset-duration`. 如果`conn-meter-reset-duration` If `conn-meter-reset-duration` is 0, incoming connections are not rate-limited.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ5NDgxMjQ1OCw0NjI2NjQwODgsLTEyMT
MzMTkyODcsMzQ4MjI0ODY4LDE0NDE5OTc4MDgsLTI0ODU1NzAz
NCwxOTkwOTUxMjY0XX0=
-->